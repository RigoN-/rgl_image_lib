...
/*
 */
Texture::Texture(int width,int height,GLuint target,int flags) : width(width), height(height), depth(1), flags(flags), target(target) {
	...
	if(flags & DXT_1) {
		...
		else if(flags & RGBA) internalformat = GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
		else fprintf(stderr,"Texture::Texture(): can't set DXT_1 compression for not RGB or RGBA format\n");
	} else if(flags & DXT_3) {
		if(flags & RGBA) internalformat = GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
		else fprintf(stderr,"Texture::Texture(): can't set DXT_3 compression for not RGBA format\n");
	} else if(flags & DXT_5) {
		if(flags & RGBA) internalformat = GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
	...
	if(target == TEXTURE_2D || target == TEXTURE_RECT) {
		...
		if(data) {
			if((flags & FLOAT_16) || (flags & FLOAT_32)) {
				...
				if(format == GL_LUMINANCE) data_float = rgba2luminance_float(data,width,height);
				else if(format == GL_LUMINANCE_ALPHA) data_float = rgba2luminance_alpha_float(data,width,height);
				else if(format == GL_RGB) data_float = rgba2rgb_float(data,width,height);
				else if(format == GL_RGBA) data_float = rgba2rgba_float(data,width,height);
...
int Texture::save(const char *name,const unsigned char *data,int width,int height) {
	...
	if(strstr(name,".jpg") || strstr(name,".JPG")) return save_jpeg(name,data,width,height,80);
...
int Texture::save_tga(const char *name,const unsigned char *data,int width,int height) {
	...
	memset(buf,0,18);
	buf[2] = 2;
	*(unsigned short*)&buf[12] = width;
	*(unsigned short*)&buf[14] = height;
	buf[16] = 32;
	buf[17] = 0x28;
	...
	fwrite(buf,sizeof(unsigned char),18 + width * height * 4,file);
	fclose(file);
	delete [] buf;
	return 1;
}
...
/*
 */
float *Texture::rgba2luminance_float(unsigned char *data,int width,int height) {
...
float *Texture::rgba2luminance_alpha_float(unsigned char *data,int width,int height) {
...
float *Texture::rgba2rgb_float(unsigned char *data,int width,int height) {
...
float *Texture::rgba2rgba_float(unsigned char *data,int width,int height) {
